...
db_total = xlrd.open_workbook('Band_assignment_data.xlsx')
title = ['memberId','age','gender','semester','talent','rank']

db = db_total.sheet_by_name('IST_data')
db_manage_total = []
for i in range(db.nrows -1):
    i+=1
    temp ={}
    for j in range(6):
        temp[title[j]]=db.row_values(i)[j]
        if j == 0 or j == 1 or j == 5:
            temp[title[j]] = int(temp[title[j]])

    db_manage_total.append(temp)
print(db_manage_total)
...
if semester_value == 1:
    band_db = open('F:\Project\bands.json', 'w')
    band_exist = []
    band_number = 0
...
def member_result(type_member, gender, rank):
    ...
    for i in range(len(type_member)):
        print(type_member[i].get('gender'))
        if type_member[i].get('gender') == gender and type_member[i].get('rank') == rank:
...
def getmember(data):
    ...
    for i in range(len(data)):
        ...
        temp = {'gender': db_manage[i].get('gender'), 'rank': db_manage[i].get('rank'), 'member_info': id_name + " || " +" || " + str(db_manage[i].get('rank'))}
                    ...
                    for t in range(2):
                        if m['talent'][t] == j:
                            if i[m['talent'][1-t]] == 'matching':
                                sum-=2
                                i[j] = m['info'][t]['memberId'] + ' || ' +m['info'][t]['firstName'] + m['info'][t]['lastName'] + ' || ' + str(m['info'][t]['rank'])
                                i[m['talent'][1-t]] =  m['info'][1-t]['memberId'] + ' || ' +m['info'][1-t]['firstName'] + m['info'][1-t]['lastName'] + ' || ' + str(m['info'][1-t]['rank'])
                                ...
                                band_gradelist[band_num][re_key[j]+7] = m['info'][t]['rank']
                                band_gradelist[band_num][re_key[m['talent'][1-t]]+7] = m['info'][t]['rank']
                                print(band_gradelist[band_num])
                                extra_request[ms] = []
                                break
    print(band_num)
